...
export function Search(props: Props) {
    ...
    async (e: ChangeEvent<HTMLInputElement>) => {
      ...
      for (let i = 0; i < files.length; ++i) {
        ...
        await mapSomeAsync(getExt(file.name), async (ext) => {
          const filename =
            file.name.split('.').slice(0, 1)[0].toLowerCase() + '.' + ext;
  ...
  return (
    <SearchList iterator={iterator} onReturn={onReturn}>
...
function useNoteSearch(search: string): IndexIterator<Note> {
  ...
  if (search)
    iter = filterIndexIterator(
      iter,
      (note) =>
        note.attributes.content.includes(search) ||
        note.attributes.tags.includes(search)
    );
  return iter;
}
...
function useTermSearch(search: string): IndexIterator<StudyDetails> {
  ...
  return useMemo(() => {
    function filteredTermIter(predicate: (t: Term) => boolean) {
      return flattenIndexIterator(
        mapIndexIterator(
          Indexer.reverseIter(
            clozeAnswers.byLastAnsweredOfNoteIdReferenceMarkerAndClozeIdx
          ),
          (clozeAnswer) => {
            ...
            return bindSome(cloze, (cloze) =>
              studyDetailsForCloze(cloze, notesIndex)
            );
          }
        )
      );
...
function useMediaSearch(search: string): IndexIterator<StoredMetadata> {
  ...
  const unsortedMediaData = useLiveQuery(
    () =>
      store.fetchMetadataByExts(
        Object.keys({ ...audioContentTypes, ...videoContentTypes })
      ),
    [],
    []
  );
...
function useSearchResults(
  ...
  const selectTermRouting = useWorkflowRouting(
    SelectTerm,
    ({ onReturn }: Props) => (
      <Search onReturn={onReturn} defaultSearch={search} defaultMode={mode} />
    ),
  ...
  const renderedTerms = useMemo(() => {
    return mapIndexIterator(termsIter, (sd) => {
      return (
        <TermSearchResult studyDetails={sd} selectRow={onApply || visitTerm} />
      );
    });
    // eslint-disable-next-line
  }, [termsIter]);
  ...
  const renderedMedia = useMemo(() => {
    return mapIndexIterator(mediaIter, (md) => (
      <MediaSearchResult
        md={md}
        selectRow={downloadFile}
