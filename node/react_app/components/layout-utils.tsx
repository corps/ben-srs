...
const classesForFlexContainer = classNamesGeneratorFor<FlexContainerOptions>(
  (add) => {
...
export function FlexContainer(
  props: PropsWithChildrenAndClassName<FlexContainerOptions>
) {
  return (
    <div
      className={classesForFlexContainer(props)}
      key={props.key}
      id={props.id}
    >
      {props.children}
...
export function Column(props: PropsWithChildrenAndClassName<ColumnOptions>) {
  return <div className={classesForColumn(props)}>{props.children}</div>;
}
...
export function Row(props: PropsWithChildrenAndClassName<RowOptions>) {
  return <div className={classesForRow(props)}>{props.children}</div>;
}

const classesForVCenteringContainer = classNamesGeneratorFor<{}>((add) => {},
<div className="dt w-100 h-100" />);

export function VCenteringContainer(props: PropsWithChildrenAndClassName<{}>) {
  return (
    <div className={classesForVCenteringContainer(props)}>{props.children}</div>
  );
}

const classesForVCentered = classNamesGeneratorFor<{}>((add) => {},
<div className="dtc v-mid" />);

export function VCentered(props: PropsWithChildrenAndClassName<{}>) {
  return <div className={classesForVCentered(props)}>{props.children}</div>;
}

const classesForVBottomed = classNamesGeneratorFor<{}>((add) => {},
<div className="dtc v-btm" />);

export function VBottomed(props: PropsWithChildrenAndClassName<{}>) {
  return <div className={classesForVBottomed(props)}>{props.children}</div>;
}
