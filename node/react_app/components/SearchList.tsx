...
export function SearchList({
  ...
  const { nextPage, data, prevPage, hasMore, page } = usePagination(
    iterator,
    perPage
  );
    ...
    (key) => {
      ...
      if (key == 'ArrowDown' || key == 'j') {
        setCursorIdx(
          some(
            withDefault(
              mapSome(cursorIdx, (i: number) => i + 1),
              0
            )
          )
        );
  ...
  const [NextWrapper] = useWithKeybinding(
    'ArrowRight',
    useCallback(() => {
      if (hasMore) nextPage();
    }, [hasMore, nextPage])
  ...
  const [PrevWrapper] = useWithKeybinding(
    'ArrowLeft',
    useCallback(() => {
      if (page > 0) prevPage();
    }, [page, prevPage])
  ...
  return (
    <div className="mw6 center">
      <div className="tc">
        ...
        {page > 0 && (
          <SimpleNavLink onClick={prevPage}>
