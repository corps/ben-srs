...
const classNames = classNamesGeneratorFor<SimpleNavLinkProps>((add) => {
  add(
    'onClick',
    <div className="pointer blue hover-light-blue" />,
    <div className="" />
  );
...
export function SimpleNavLink(
  props: PropsWithChildrenAndClassName<SimpleNavLinkProps>
) {
  ...
  return (
    <span onClick={onClick} className={classNames(props)}>
      {props.children}
    </span>
  );
}
    ...
    <>
      {onApply ? (
        <SimpleNavLink
          ...
          onClick={applyDisabled ? null : onApply}
