...
function requestMore<V>(
  valueIterator: IndexIterator<V>,
  pp: number,
  { page, pagedData, nextValue }: State<V>
) {
  let n = nextValue;
...
export function usePagination<V, R>(
  valueIterator: IndexIterator<V>,
  perPage: number
) {
  ...
  const data = useMemo(
    () => pagedData.slice(page * perPage, (page + 1) * perPage),
    [page, pagedData, perPage]
  );
