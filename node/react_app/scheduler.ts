...
export function configuredScheduler(random = () => Math.random()) {
  function intervalFrom(
    ...
    let earlyAnswerMultiplier = Math.min(
      1.0,
      answeredInterval / currentInterval
    );
    earlyAnswerMultiplier =
      1 - Math.sin((Math.PI / 2) * (1 - earlyAnswerMultiplier));

    const effectiveFactor =
      baseFactor + bonusFactor * earlyAnswerMultiplier * randomFactor;
    const nextInterval = Math.max(
      currentInterval * effectiveFactor,
      minimalIntervalOf(willRemainNew)
    );
  ...
  return (
    ...
    delayFactor = null as number | null
  ) => {
    ...
    const nextInterval = intervalFrom(
      factor,
    ...
    if (delayFactor) {
      const nextDelayInterval = intervalFrom(
        delayFactor,
        schedule.delayIntervalMinutes || 0,
        Infinity,
        willRemainNew,
        wasNew
      );
...
export function medianSchedule(existingSchedules: Schedule[]) {
  ...
  const summedInterval = existingSchedules.reduce(
    (sum, sch) => sum + sch.intervalMinutes,
    0
  );
