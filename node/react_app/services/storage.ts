...
export const dirtyStoreIndexer = new Indexer<StoredMedia, DirtyStoreIndex>(
  'byId'
);
...
export class FileStore {
  ...
  async markDeleted(id: string) {
    let storedMetadata: StoredMetadata = await this.db
      .table('metadata')
      .where('id')
      .equals(id)
      .first();
  ...
  private async storeBlobAndMetadata(
    ...
    if (storedMetadata.dirty) {
      ...
      this.dirtyIndex = dirtyStoreIndexer.removeByPk(this.dirtyIndex, [
        media.id
      ]);
    }
  ...
  async deletePath(path: string): Promise<void> {
    await this.metaLoaded;
    this.dirtyIndex = dirtyStoreIndexer.removeAll(
      this.dirtyIndex,
      Indexer.getAllMatching(this.dirtyIndex.byPath, path.split('/'))
    );
  ...
  async fetchBlobsByExt(ext: string): Promise<StoredMedia[]> {
    return this.db
      ...
      .equals(ext)
      .toArray()
      .then((media) => this.replaceFromDirty(media));
  }
