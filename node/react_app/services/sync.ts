  ...
  for (let d of dirty) {
    if (d.deleted) {
      ...
      for (let work of backend.uploadFile(d)) {
        const result: Either<FileMetadata, 'conflict'> = yield* runPromise(
          work
        );
        const conflict: Maybe<'conflict'> = getLeft<FileMetadata, 'conflict'>(
          result
        );
        ...
        yield* runPromise(
          withDefault(
            mapSome(updated, (updated) => {
              const updatedSf: StoredMedia = { ...d, ...updated };
              return storage.storeBlob(
                normalizeBlob(updatedSf.blob),
                updatedSf,
                false
              );
            }),
        ...
        yield* handleConflict(
          Promise.resolve<Maybe<'conflict'>>(conflict as any),
          d
        );
      }
