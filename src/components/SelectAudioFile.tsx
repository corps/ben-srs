...
export function SelectAudioFile(props: Props) {
  ...
  const curSelectedAudioPath = useMemo(() => allAudioMetadatas.find(({id}) => id === value)?.path || "", [allAudioMetadatas, value]);
  const audioPaths = useMemo(() => ["<unset>", ...options.map(({path}) => path)], [options]);
