...
export function useAllTags(language: string, singular?: boolean) {
  ...
  const tagsInIndex =  useMemo(() =>
    Indexer.getAllMatching(notesIndex.taggedNotes.byLangAndTagOfFirstNoteId, [language]).filter(({tag}) => tag !== language || singular).map(({tag}) => tag)
...
export function TagsSelector({value, language, onChange, singular}: PropsWithChildren<Props>) {
  ...
  return <div className="f5">
    タグ:
    {allTags.map((tag, i) => <div className="ml2 w4 dib">
      <SelectSingle
