...
Object.defineProperty(exports, "__esModule", { value: true });
var jd = __webpack_require__(/*! json-delta */ 0);
if (typeof importScripts === "function") {
    run();
}
function run() {
    var openReq = indexedDB.open("ledger-storage", 1);
    var db;
    var queuedRequests = [];
    var keyStates = {};
    var queuedWrite = {};
    var writing = {};
    var requestedSaveValues = {};
    ...
    function doWrite(db, key, value, state) {
        ...
        var req = index.openKeyCursor(IDBKeyRange.upperBound([key, Number.MAX_VALUE]), "prev");
        req.onsuccess = function (e) {
            var cursor = e.target.result;
            var req;
            if (cursor && cursor.key[1] === state.curSeq) {
                var startDiff = Date.now();
                var diff = jd.diff(state.curVal, value, 30);
                console.log("putting diff", key, diff, Date.now() - startDiff);
                req = store.put({ key: key, diff: diff });
            }
            else {
                console.log("putting reet diff", key, value);
                req = store.put({ key: key, diff: [[[], value]] });
            }
            req.onsuccess = function (e) {
        ...
        tx.oncomplete = function () {
            ...
            var queuedValue = queuedWrite[key];
    ...
    function runRequest(db, request) {
        switch (request.type) {
            case "clear":
                ...
                objStoreReq.onerror = function (e) {
                    console.error(e);
                };
                tx.oncomplete = function () {
                    keyStates = {};
                ...
                break;
            case "save":
                var key = request.key, diff = request.diff;
                ...
                break;
            case "load":
                var key = request.key, readSeq = request.readSeq;
                var tx = db.transaction(STORE_NAME, "readwrite");
                var store = tx.objectStore(STORE_NAME);
                var index = store.index(INDEX_NAME);
                var req = index.openCursor(IDBKeyRange.bound([key, 0], [key, Number.MAX_VALUE], true, true));
                var result = null;
                ...
                req.onsuccess = function (e) {
                    ...
                    if (cursor) {
                        ...
                        req = store.put({ key: key, diff: [[[], result]] });
                        req.onsuccess = function (e) {
                            state.curSeq = req.result;
                            state.curVal = result;
                        ...
                        reply({ key: key, readSeq: readSeq, value: result });
                    }
                };
                req.onerror = function (e) {
                    console.error(e);
                    reply({ key: key, readSeq: readSeq, value: null });
                };
                break;
        }
    }
